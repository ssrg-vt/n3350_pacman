...
efi_main (EFI_HANDLE image, EFI_SYSTEM_TABLE *SystemTable)
{
    ...
    Print(L"BagguBounds: Malloc Patch ...  Lalit\n");
    ...
    state.len = (1000 * sizeof(double)) + 3;
 	state.scalar = 3.0;
    ...
    // Stream test--         //** -----
    // Stream test--         
	// Stream test--         for (i = 0; i < state.len; ++i) {
	// Stream test--         	state.a[i] = 0xDEADBEEF;
	// Stream test--         	state.b[i] = 0xDEADBEEF;
	// Stream test--         	state.c[i] = 0xDEADBEEF;
	// Stream test--         }
    // Stream test-- Print(L"Second      [%02d]\n", Time.Second);
    // Stream test-- Print(L"Nanosecond  [%d]\n",   Time.Nanosecond);

    // Stream test--         start_ns =  Time.Nanosecond; start_sec =  Time.Second;
	// Stream test--         for (i = 0; i < state.len; ++i) {
	// Stream test--         	state.a[i] = 0x1;
	// Stream test--         	state.b[i] = 0x2;
	// Stream test--         	state.c[i] = 0xF;
	// Stream test--         }
    // Stream test--         end_ns =  Time.Nanosecond;  end_sec =  Time.Second;
    // Stream test--         ns = end_ns-start_ns; seconds = end_sec-start_sec;
    // Stream test--        
    // Stream test--         bw = (((state.len*3*8)/1024)/1024)/seconds;
    // Stream test--         Print(L"-------------------------------------\n");
    // Stream test--         Print(L"STREAM fill latency : %d nanoseconds \n", ns);
    // Stream test--         Print(L"STREAM fill bandwidth : %04f MB/s\n", bw);
    // Stream test-- Print(L"Second      [%ld]\n", Time.Second);
    // Stream test-- Print(L"Nanosecond  [%ld]\n",   Time.Nanosecond);

    // Stream test--         ns=0; seconds=0;start_ns =  Time.Nanosecond; start_sec =  Time.Second;
	// Stream test--         for (i = 0; i < state.len; ++i) {
	// Stream test--         	state.c[i] = state.b[i];
	// Stream test--         	state.b[i] = state.a[i];
	// Stream test--         	state.a[i] = state.c[i];
	// Stream test--         }
    // Stream test--         end_ns =  Time.Nanosecond;  end_sec =  Time.Second;
    // Stream test--         ns = end_ns-start_ns; seconds = end_sec-start_sec;

    // Stream test--         bw = (((state.len*3*8)/1024)/1024)/seconds;
    // Stream test--         Print(L"STREAM copy latency : %ld nanoseconds \n", ns);
    // Stream test--         Print(L"STREAM copy bandwidth : %ld MB/s\n", bw);
    // Stream test-- Print(L"Second      [%04f]\n", Time.Second);
    // Stream test-- Print(L"Nanosecond  [%04f]\n",   Time.Nanosecond);

    // Stream test--         ns=0; seconds=0;start_ns =  Time.Nanosecond; start_sec =  Time.Second;
	// Stream test--         for (i = 0; i < state.len; ++i) {
	// Stream test--         	//-- state.c[i] = state.a[i] + state.b[i];
	// Stream test--         	//-- state.b[i] = state.a[i] + state.c[i];
	// Stream test--         	state.a[i] = state.b[i] + state.c[i];
	// Stream test--         }
    // Stream test--         end_ns =  Time.Nanosecond;  end_sec =  Time.Second;
    // Stream test--         ns = end_ns-start_ns; seconds = end_sec-start_sec;
    // Stream test--         bw = (((state.len*3*8)/1024)/1024)/seconds;

    // Stream test--         Print(L"STREAM add latency : %ld nanoseconds \n", ns);
    // Stream test--         Print(L"STREAM add bandwidth : %d MB/s\n", bw);
    // Stream test-- Print(L"Second      [%02d]\n", Time.Second);
    // Stream test-- Print(L"Nanosecond  [%d]\n",   Time.Nanosecond);

    // Stream test--         ns=0; seconds=0;start_ns =  Time.Nanosecond; start_sec =  Time.Second;
	// Stream test--         for (i = 0; i < state.len; ++i) {
	// Stream test--         	state.a[i] = state.a[i] + state.scalar*state.b[i];
	// Stream test--         	//-- state.b[i] = state.a[i] + state.scalar*state.c[i];
	// Stream test--         	//-- state.a[i] = state.b[i] + state.scalar*state.c[i];
	// Stream test--         }
    // Stream test--         end_ns =  Time.Nanosecond;  end_sec =  Time.Second;
    // Stream test--         ns = end_ns-start_ns; seconds = end_sec-start_sec;
    // Stream test--         bw = (((state.len*3*8)/1024)/1024)/seconds;

    //-- stream test--         Print(L"STREAM triad latency : %ld nanoseconds \n", seconds);
    //-- stream test--         Print(L"STREAM triad bandwidth : %d MB/s\n", bw);
    //-- stream test-- Print(L"Second      [%02d]\n", Time.Second);
    //-- stream test-- Print(L"Nanosecond  [%d]\n",   Time.Nanosecond);
    //-- stream test--         Print(L"-------------------------------------\n");
