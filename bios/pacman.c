...
efi_main (EFI_HANDLE image, EFI_SYSTEM_TABLE *SystemTable)
{
    ...
    if (EFI_ERROR(status)) {
        Print(L"Usage:\n");
        Print(L"  patch:        <tool> p\n");
        Print(L"  patch & exec: <tool> x\n");
        Print(L"  perf:         <tool> f\n");
        Print(L"  zero out m&p: <tool> z\n");
        Print(L"  hook:         <tool> h  [m&p idx] [uop addr] [patch addr]\n");
        Print(L"  template:     <tool> m\n");
        Print(L"  dump imms:    <tool> di\n");
        Print(L"  dump rom:     <tool> dr\n");
        Print(L"  dump msrs:    <tool> dm\n");
        Print(L"  dump SMM:     <tool> ds [address] [size]\n");
        Print(L"  cpuid:        <tool> c  [rax] [rcx]\n");
        Print(L"  rdmsr:        <tool> rm [msr]\n");
        Print(L"  wrmsr:        <tool> wm [msr]\n");
        Print(L"  read:         <tool> r  [cmd] [addr]\n");
        Print(L"  write:        <tool> w  [cmd] [addr] [value]\n");
        Print(L"  invoke:       <tool> i  [addr]\n");
        Print(L"  update ucode: <tool> u  [size]\n");
        Print(L"  ldat read:    <tool> lr [port] [array] [bank] [idx] [addr] [optional size]\n");
        Print(L"  ldat write:   <tool> lw [port] [array] [bank] [idx] [addr] [value]\n");
        Print(L"ERROR: Parsing command line arguments: %d\n", status);
        Print(L"Efi exe hardcoded to test PAC-patch : Lalit \n");
        Print(L"Applying x86 PAC patch and Testing PACMAN Gadget.... \n");
        test_PACMAN(); 
        Print(L"x86 PAC patch and PACMAN test... Done\n");
        return status;
    }
